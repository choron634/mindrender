...
public class HelicopterAgent_seperate2_with_buildings : Agent
{
   ...
   // private int[] Points;
    ...
    private void Start() {
        ...
        Distance_to_next_waypoint = PositionSensor.GetDistance()[0];
  ...
  /*  public void FixedUpdate() {
    ...
    */
    ...
    public override List<float> CollectSensorObservations()
    {
        var sensorobservations = new List<float>();
        // センサーの距離をリストに追加する
        Array.ForEach(Sensors, sensor => {
            sensorobservations.AddRange(sensor.Hits());
        });
    ...
    public override List<float> CollectObservations()//その他に関する情報を取得
    {
        ...
        float velocityangle = PositionSensor.GetVelocityAngles()[GoalCounter];
        ...
        //ウェイポイントへのベクトルを取得
        observations.Add(PositionSensor.GetVector()[GoalCounter].x);

        observations.Add(PositionSensor.GetVector()[GoalCounter].y);

        observations.Add(PositionSensor.GetVector()[GoalCounter].z);
    ...
    public override void AgentAction(float[] outputs)//動かす
    {
        ...
        if (ReachWaypoint())//ウェイポイントに到達したときの処理
        {
            ...
            ExGoal = CurrentGoal;
            CurrentGoal = PositionSensor.points[GoalCounter+1].transform.position;
            Distance_to_next_waypoint = PositionSensor.GetDistance()[GoalCounter+1];
        ...
        if (GoalCounter < PointNumber)
        {
            sphere = GameObject.Find("Sphere" + GoalCounter);
        ...
        float waypointdistance = PositionSensor.GetDistance()[GoalCounter];
        int waypointobject = PositionSensor.GetObjects()[GoalCounter];
        float horizontalangle = PositionSensor.GetHorizontalAngles()[GoalCounter];
        ...
        */
        if (transform.position.y <= 2.0f)//地面に接触し続けるものを消す
        {
            AddReward(-100f);
            Controller.Stop();
            Done();
        ...
        */
        ...
        var deltay = 1-Mathf.Pow(Mathf.Min((Math.Abs(PositionSensor.GetVector()[GoalCounter].y)/100),1),3);
        var d = 5*(1-Mathf.Pow(Mathf.Min(PositionSensor.GetDistance()[GoalCounter] /Distance_to_next_waypoint,1), 3));
        
        var angle =1- (Math.Abs(velocityangle));

        AddReward(d - Mathf.Min(CollisionCount/10,1));//ウェイポイントまでの距離が近いほうが良い
